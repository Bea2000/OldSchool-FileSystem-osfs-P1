...
MTH= -lm  # Nota: Comentado porque no se usa
...
simdiskformat.bin: | simdiskformat.zip
	unzip -o simdiskformat.zip

simdiskfilled.bin: | simdiskfilled.zip
	unzip -o simdiskfilled.zip

simdiskformat.zip:
	scp ssoo:/home/iic2333/P0/simdiskformat.zip .

simdiskfilled.zip:
	scp ssoo:/home/iic2333/P0/simdiskfilled.zip .

.PHONY: download-disks upload test

download-disks: simdiskformat.bin simdiskfilled.bin

upload:
	rsync --recursive --delete --update --itemize-changes --human-readable --progress src Makefile ssoo:~/P0

test:
	valgrind ./osfs simdiskfilled.bin
